...
export default function Hero({ projectCount }) {
  ...
  return (
    ...
    >
      {/* Ambient glows */}
      <div className="pointer-events-none absolute top-[8%] right-[4%] w-[520px] h-[520px] rounded-full"
           style={{ background: 'radial-gradient(circle, rgba(129,140,248,0.13) 0%, transparent 70%)' }} />
      <div className="pointer-events-none absolute bottom-[4%] -left-[4%] w-[380px] h-[380px] rounded-full"
           style={{ background: 'radial-gradient(circle, rgba(100,255,218,0.08) 0%, transparent 70%)' }} />
      ...
      >
        ...
        <motion.div variants={fadeUp} className="mb-7">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full
                ...
                style={{ background: 'rgba(100,255,218,0.06)', borderColor: 'rgba(100,255,218,0.22)', color: '#64ffda' }}>
        ...
        <motion.div variants={fadeUp} className="flex flex-wrap gap-4 mb-9">
          ...
          <a href="/api/cv/download" download
        ...
        {/* Socials */}
        <motion.div variants={fadeUp} className="flex gap-3 pb-6 border-b"
                    style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
          {[
            { icon: <Github size={18} />,   href: 'https://github.com',    label: 'GitHub'   },
            { icon: <Linkedin size={18} />, href: 'https://linkedin.com',  label: 'LinkedIn' },
            { icon: <Mail size={18} />,     href: 'mailto:your@email.com', label: 'Email'    },
          ].map(s => (
            <a key={s.label} href={s.href} target="_blank" rel="noreferrer" aria-label={s.label}
               ...
               style={{ background: '#10161f', borderColor: 'rgba(255,255,255,0.07)', color: '#94a3b8' }}>
              {s.icon}
            </a>
          ))}
        </motion.div>
        ...
        <motion.div variants={fadeUp} className="flex gap-10 pt-6">
          {[
            ...
            <div key={s.label}>
              <div className="font-display font-extrabold text-[2.1rem] leading-none" style={{ color: '#64ffda' }}>
                {s.value}
              </div>
              <div className="mt-1 text-[0.72rem] uppercase tracking-[0.09em]" style={{ color: '#475569' }}>
                {s.label}
              </div>
            </div>
          ))}
      ...
      >
        <div className="relative w-[400px] h-[440px]">
          ...
          <div className="absolute -inset-[3px] rounded-[26px] z-0"
               style={{
                 background: 'conic-gradient(from 0deg, #64ffda, #818cf8, transparent 50%, #64ffda)',
                 animation: 'spin-slow 8s linear infinite',
               }} />

          {/* Dark backing plate */}
          <div className="absolute inset-[2px] rounded-[24px] z-[1]" style={{ background: '#07090e' }} />
          ...
          <div className="absolute inset-[5px] z-[2] rounded-[20px] overflow-hidden group">
            ...
            <div className="absolute bottom-0 left-0 right-0 h-24 pointer-events-none"
                 style={{ background: 'linear-gradient(to top, rgba(7,9,14,0.65) 0%, transparent 100%)' }} />
          </div>

          {/* Bottom glow */}
          <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 w-3/4 h-12 z-0 blur-2xl pointer-events-none"
               style={{ background: 'radial-gradient(ellipse, rgba(100,255,218,0.28), transparent 70%)' }} />
        </div>
        ...
        <CyclingFloatChip
          ...
          roles={[
            { emoji: '⚡', label: 'Specialty', value: 'Full-Stack Dev',  valueColor: '#e2e8f0' },
            { emoji: '🤖', label: 'Specialty', value: 'Junior ML Eng',   valueColor: '#818cf8' },
            { emoji: '🎨', label: 'Specialty', value: 'UI / UX Dev',     valueColor: '#64ffda' },
          ]}
        />

        {/* Float chip — bottom left */}
        <FloatChip emoji="🚀" label="Status" value="Open to Offers" valueColor="#64ffda"
...
function FloatChip({ emoji, label, value, valueColor, className = '', style = {} }) {
  return (
    <div
    ...
    >
      ...
      <div>
        <div className="text-[0.61rem] uppercase tracking-wide mb-0.5" style={{ color: '#475569' }}>
          {label}
        </div>
        <div className="text-[0.82rem] font-semibold leading-none"
  ...
  return (
    ...
    >
      ...
      <div style={{ overflow: 'hidden' }}>
        {/* Label */}
        <div className="text-[0.61rem] uppercase tracking-wide mb-0.5"
             style={{ color: '#475569' }}>
          {current.label}
        </div>
